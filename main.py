    ...
    if data.startswith("call_admin_"):
        ...
        try:
            with open("logs/bot.log", "a", encoding="utf-8") as f:
                f.write(f"{datetime.now().isoformat()} - 📞 ПОПЫТКА ЗВОНКА: {phone}\n")
        except Exception as e:
            print(f"Ошибка записи в лог: {e}")
        
        # ← УНИВЕРСАЛЬНОЕ ФОРМАТИРОВАНИЕ ДЛЯ ВСЕХ СТРАН И УСТРОЙСТВ
        # phone приходит как строка цифр без пробелов
        
        # 1. Сначала определим, какой номер пришел
        if len(phone) == 11 and phone.startswith("8"):
            # Российский номер: 89526097380
            display_phone = f"+7 {phone[1:4]} {phone[4:7]}-{phone[7:9]}-{phone[9:11]}"  # +7 952 609-73-80
            tel_phone = f"+7{phone[1:]}"  # +79526097380
            clickable_phone = tel_phone   # Для клика
        elif len(phone) == 12 and phone.startswith("7"):
            # Российский номер в формате 7XXXXXXXXXX
            display_phone = f"+7 {phone[1:4]} {phone[4:7]}-{phone[7:9]}-{phone[9:12]}"
            tel_phone = f"+{phone}"  # +7XXXXXXXXXX
            clickable_phone = tel_phone
        elif phone.startswith("+"):
            # Уже международный формат: +380501234567
            display_phone = phone
            tel_phone = phone
            clickable_phone = phone
        else:
            # Любой другой формат (может быть короткий номер)
            display_phone = phone
            tel_phone = phone
            clickable_phone = phone
        
        # 2. Ссылка для звонка - ПРОСТО НОМЕР (работает везде!)
        # Telegram на iOS/Android автоматически делает номера кликабельными
        call_url = f"tel:{clickable_phone}"
        
        # 3. Записываем попытку звонка
        ...
        # 4. СООБЩЕНИЕ ДЛЯ ПОЛЬЗОВАТЕЛЯ (УНИВЕРСАЛЬНОЕ)
        # Telegram на мобильных делает номера кликабельными даже без <a href>
        message_text = (
            f"📞 <b>Для звонка администратору:</b>\n\n"
            f"{clickable_phone}\n\n"
            f"<i>Нажмите на номер для звонка (на мобильных устройствах).</i>\n\n"
            f"Если администратор не ответит, мы уведомим его о пропущенном звонке.\n\n"
            f"Или оставьте сообщение:"
        )
        
        # 5. HTML-ссылка для настольных версий
        html_message = (
            f"📞 <b>Для звонка администратору:</b>\n\n"
            f"<a href='{call_url}'>{display_phone}</a>\n\n"
            f"<i>Нажмите на ссылку для звонка.</i>\n\n"
            f"Если администратор не ответит, мы уведомим его о пропущенном звонке.\n\n"
            f"Или оставьте сообщение:"
        )
