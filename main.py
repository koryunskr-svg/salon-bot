...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("subservice_"):
        ...
        # --- НАЧАЛО ИЗМЕНЕНИЯ button_handler для priority ---
    if data.startswith("priority_"):
        ...
        if priority_choice == "date":
            ...
            # Удаляем возможную дату, выбранную ранее (например, при возврате назад)
            context.user_data.pop("date", None)
            # Удаляем возможного мастера, выбранного ранее (например, при возврате назад)
            context.user_data.pop("selected_specialist", None)
            # Вызываем select_specialist, который теперь будет искать мастеров без даты
            ...
            return
    # --- /КОНЕЦ ИЗМЕНЕНИЯ button_handler для priority ---
elif priority_choice == "specialist":
    # Сценарий B: Сначала мастер -> потом дата
    # Удаляем возможную дату, выбранную ранее (например, при возврате назад)
    context.user_data.pop("date", None)
    # Удаляем возможного мастера, выбранного ранее (например, при возврате назад)
    context.user_data.pop("specialist", None)
    # Вызываем select_specialist, который теперь будет искать мастеров без даты
    return await select_specialist(update, context)
else:
    # На всякий случай
    ...
    for row in all_services:
        ...
        if len(row) > 1 and row[1] == ss:
            ...
            try:
                ...
                logger.warning(f"⚠️ Неверный формат длительности/буфера в услуге {ss}: {row[2]}, {row[3]}")
            ...
            if len(row) > 6: # Если индекс 6 существует (колонка G - Описание)
                description = row[6].strip() # Берём значение и убираем лишние пробелы
            # --- /ПОЛУЧАЕМ ОПИСАНИЕ ---
...
# --- SELECT DATE ---
# --- НАЧАЛО ПОЛНОЙ ЗАМЕНЫ select_date ---
async def select_date(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    specialist = context.user_data.get("selected_specialist") # Может быть None
    ...
    if specialist:
        ...
        for days_offset in range(days_ahead + 1):
            ...
            for row in schedule_data:
                if len(row) > 0 and row[0].strip() == specialist:
                    spec_schedule_row = row
                    break

            if not spec_schedule_row:
                logger.warning(f"⚠️ График для специалиста '{specialist}' не найден.")
            ...
            if day_index < len(spec_schedule_row):
                ...
                if work_schedule.lower() != "выходной" and work_schedule:
                    # Здесь нужно проверить, есть ли у специалиста *свободное окно* в этот день для `service_duration + service_buffer`
                    # Это требует вызова логики из select_time или её дублирования.
        ...
        await query.edit_message_text(f"📅 Выберите дату для специалиста '{specialist}':", reply_markup=InlineKeyboardMarkup(kb))
        context.user_data["state"] = SELECT_DATE
        return

    # --- СЦЕНАРИЙ A: "Сначала дата", потом мастер ---
    else: # specialist is None
        ...
        context.user_data["state"] = SELECT_DATE
        return
# --- /КОНЕЦ ПОЛНОЙ ЗАМЕНЫ select_date ---

# --- SELECT SPECIALIST ---
# --- НАЧАЛО ПОЛНОЙ ЗАМЕНЫ select_specialist ---
async def select_specialist(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # --- СЦЕНАРИЙ B: "Сначала мастер", потом дата ---
        ...
        context.user_data["state"] = SELECT_SPECIALIST
        return
# --- /КОНЕЦ ПОЛНОЙ ЗАМЕНЫ select_specialist ---
# --- SELECT TIME ---
