...
def load_admins():
    ...
    try:
        # Читаем с A3, предполагая, что A1 — название листа, A2 - заголовки
...
async def notify_admins(context, message: str):
    ...
    for chat_id in ADMIN_CHAT_IDS:
        try:
            ...
            logger.info(f"📤 Уведомление админу {chat_id}: {message[:50]}...") # Логируем начало сообщения
        except Exception as e:
            logger.exception(f"❌ Не удалось отправить админу {chat_id} сообщение: {message[:50]}..., ошибка: {e}")
