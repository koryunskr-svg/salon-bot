...
def validate_phone(phone_str: str) -> str:
    ...
    print(f"🔧 validate_phone: '{phone_str}' → цифры: '{digits_only}', длина: {len(digits_only)}")
    ...
    # МЕЖДУНАРОДНЫЕ НОМЕРА (не начинаются с 7 или 8)
    print(f"🔧 validate_phone: Международный: '{digits_only}'")
    return digits_only
